...
pub async fn spawn(session: &Session) -> Result<JoinHandle<()>> {
    let publisher = session
        ...
        .await
        .map_err(|e| anyhow::anyhow!("Failed to declare temperature publisher: {}", e))?;
    ...
    let handle = tokio::spawn(async move {
        ...
        let backend = match PosixShmProviderBackend::builder(65536).wait() {
            Ok(b) => b,
            Err(e) => {
                eprintln!("[Publisher] Failed to create SHM backend: {}, falling back to regular publish mode", e);
        ...
        let layout = match shm_provider.alloc_layout(256) {
            Ok(l) => l,
            Err(e) => {
                eprintln!("[Publisher] Failed to create SHM allocation layout: {}, falling back to regular publish mode", e);
                run_without_shm(publisher).await;
                return;
            }
        };

        println!("[Publisher] Shared memory mode enabled");
        ...
        loop {
            ...
            println!("[Publisher] Publishing: {}", msg);
                ...
                .await
            {
                ...
                Err(e) => {
                    eprintln!("[Publisher] SHM allocation failed: {:?}, using regular publish", e);
